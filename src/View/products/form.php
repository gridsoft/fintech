<?php $action = $product !== null ? "/products/{$product->id}" : '/products'; ?>
    ...
    <div class="card-body">
        <form method="post" action="<?= $action ?>" class="row g-3" style="max-width: 640px;">
            ...
            <div class="col-md-4">
                ...
                <select id="category_id" name="category_id" class="form-select <?= isset($errors['category_id']) ? 'is-invalid' : '' ?>">
                    ...
                    <?php foreach ($categories as $category): ?>
                        <option value="<?= $category->id ?>" <?= (int) ($product->categoryId ?? 0) === $category->id ? 'selected' : '' ?>>
                ...
                <?php else: ?>
                    <div class="form-text">Нема категории? <a href="/product-categories/create" target="_blank">Создади една →</a></div>
                <?php endif; ?>
            </div>
            ...
            <div class="col-12">
                ...
                <a href="/products" class="btn btn-outline-secondary">Откажи</a>
            </div>
        </form>
    </div>
</div>
