<div class="mb-3">
    <a href="/accounts" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left"></i> Назад кон контен план</a>
</div>
    ...
    <div class="table-responsive">
        <table class="table align-middle mb-0">
            ...
            <tbody>
                <?php if (empty($rows)): ?>
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">Нема книжења на оваа сметка.</td>
                    </tr>
                <?php endif; ?>
                <?php foreach ($rows as $row): ?>
                    <tr class="cursor-pointer" onclick="location.href='/journal/<?= $row['entry']->id ?>'" style="cursor:pointer;">
                        ...
                        <td class="text-muted"><?= htmlspecialchars($row['entry']->reference ?? '—') ?></td>
